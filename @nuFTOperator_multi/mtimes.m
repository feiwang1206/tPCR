...
if isa(A,'nuFTOperator_multi')
    ...
    if A.adjoint
        ...
        for k=1:A.numCoils
            for m=1:A.shot
                Q = Q + nufft_adj(B(((k-1)*A.shot+m-1)*A.trajectory_length+1:((k-1)*A.shot+m)*A.trajectory_length), A.nufftStruct{m}) .* conj(A.sensmaps{k});
            end
            % Normalization
        end
        Q = Q / dim;
    ...
    % coil sensitivities. Thereafter the nuFFT is applied
        ...
        Q = zeros(A.trajectory_length*A.numCoils*A.shot,1);
        for m=1:A.shot
            for k=1:A.numCoils
                Q(((k-1)*A.shot+m-1)*A.trajectory_length+1:((k-1)*A.shot+m)*A.trajectory_length) = nufft((B.*A.sensmaps{k}), A.nufftStruct{m});
            end
        end
        Q = Q / dim;
    end
