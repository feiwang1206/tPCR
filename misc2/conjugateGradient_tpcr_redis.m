function [z,z_iter] = conjugateGradient_tpcr_redis(A,b,acc,tol,tol1,maxit,z0,verbose_flag, newfigure)
...
%         fprintf(['Iteration #', num2str(n),': current tolerance = ', num2str(sqrt(normrr/normrr0)), '\n']);
    ...
    z_iter{3}(n)=sqrt(normrr/normrr0);
    z_iter{5}(n)=(l2norm(alpha*d)/reference_norm);
end
z_iter{1}=z;
