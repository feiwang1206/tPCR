function [z, z_iter] = regularizedReconstruction_tpcr(A,b,acc,tol1,varargin)
...
for k=1:length(I_fhandle)
    linear_flag = linear_flag * or(strcmp(func2str(varargin{I_fhandle(k)}),'L2Norm'),...
        strcmp(func2str(varargin{I_fhandle(k)}),'L2Norm_n'));
end
...
%         z0 = z0 / gamma; % adjust the amplitude of the initial guess according to the data 'b'
    ...
    reference_norm = 2*l2norm(A'*b);
    
    [f,df] = cost_function;
    
    if nargout==1
        [z] = nonlinearConjugateGradient_tpcr_redis(f,df,z0,acc,tol,tol1,maxit,cg_method,verbose_flag,reference_norm);
    else
        [z, z_iter] = nonlinearConjugateGradient_tpcr_redis(f,df,z0,acc,tol,tol1,maxit,cg_method,verbose_flag,reference_norm);
    end
...
%     toc
    if nargout==1
        z = conjugateGradient_tpcr_redis(S,q,acc,tol,tol1,maxit,z0,verbose_flag,newfigure);
    else
        [z,z_iter] = conjugateGradient_tpcr_redis(S,q,acc,tol,tol1,maxit,z0,verbose_flag,newfigure);
    end    
end
