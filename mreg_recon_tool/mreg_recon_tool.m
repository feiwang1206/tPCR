...
D.gui_data.temporal_recon_type_string = {'standard','standard_combine','tPCR_step1','tPCR_step2','tPCR_step3'};
...
H.tPCR_acc_rate_t = uicontrol(H.main_fig,'Style','Text','Parent',H.recon_panel,'String','tPCR_acc_rate','Position',[spacing,p(2)-tb_height-spacing,pb_width,tb_height],'HorizontalAlignment','left');
H.tPCR_acc_rate_e = uicontrol(H.main_fig,'Style','Edit','Parent',H.recon_panel,'String','0.1','Position',[spacing,p(2)-pb_height-tb_height-spacing,pb_width,pb_height],'BackgroundColor','white','Callback',{@tPCR_acc_rate_e_cb},'Callback',{@tPCR_acc_rate_e_cb});
p = get(H.tPCR_acc_rate_e,'Position');
...
D.recon.recon_details.tPCR_acc_rate = str2num(get(H.tPCR_acc_rate_e,'String'));
...
if nargin==1
    ...
    if ~isempty(settings.gui_data.rawdata_filename) && ~isempty(settings.gui_data.reference_filename) && ~isempty(settings.gui_data.trajectory_filename)
        ...
        recon_voxel_size_e_cb([],[],settings.recon.recon_details.tPCR_acc_rate);
...
%%%%%%%%%%%%
    function temporal_recon_type_pm_cb(hobj, eventdata, str)
        ...
        if strcmp(str,'tPCR_step1') || strcmp(str,'tPCR_step2') || strcmp(str,'tPCR_step3')
            set(H.tPCR_acc_rate_e,'Enable','on');
            set(H.save_during_recon_cb,'Enable','off','Visible','off');
        else
            set(H.tPCR_acc_rate_e,'Enable','off');
...
%             if exist([D.recon.recon_details.pname '/recon_details.mat'],'file')
%                 d=load([D.recon.recon_details.pname '/recon_details.mat']);
%                 D.recon.recon_details.DORK_frequency=d.recon_details.DORK_frequency;
%                 D.recon.recon_details.DORK_phi_offset=d.recon_details.DORK_phi_offset;
%             else
...
%             end
            recon_details=D.recon.recon_details;
            save([D.recon.recon_details.pname '/recon_details.mat'],'recon_details');
...
%%%%%%%%%%%%
    function tPCR_acc_rate_e_cb(hobj,eventdata,val)
        if nargin<=2
            rate = str2num(get(H.tPCR_acc_rate_e,'String'));
        else
            set(H.tPCR_acc_rate_e,'String',num2str(val,'%i'));
            rate = val;
        end
        
        if rate>1 || rate<0
            rate=1;
            D.recon.recon_details.tPCR_acc_rate = rate;
            set(H.tPCR_acc_rate_e,'String',num2str(rate));
            local_set_status(['tPCR_acc_rate must be from 0 to 1, tPCR accelerate rate set to ',num2str(D.recon.recon_details.tPCR_acc_rate*100),'%.']);
        else
            D.recon.recon_details.tPCR_acc_rate = rate;
            local_set_status(['tPCR accelerate rate set to ',num2str(D.recon.recon_details.tPCR_acc_rate*100),'%.']);
        end
    end
%%%%%%%%%%%%
    function use_gridengine_cb_cb(hobj, eventdata, val)
        ...
        D.recon.recon_details.EnableSlurm = D.param.use_gridengine;
...
%%%%%%%%%%%%
    function frames_per_job_e_cb(hobj,eventdata,val)
        ...
        D.recon.recon_details.frames_per_job = frames_per_job;
...
%%
        ...
        D.recon.recon_details.frames_per_job=D.param.frames_per_job;
...
%%%%%%%%%%%%
    function recon_pb_cb(hobj,eventdata)
        if all([D.check.rawdata_loaded_flag, D.check.reference_loaded_flag, D.check.trajectory_loaded_flag])
            ...
            D.recon.recon_details.pname = fullfile(D.gui_data.working_directory,...
                [D.recon.recon_details.penalty.norm_string '_' ...
                num2str(D.recon.recon_details.penalty.lambda)]);
            mkdir(D.recon.recon_details.pname);
            if D.check.adjust_reference_and_trajectory_flag==0 && ~exist([D.recon.recon_details.pname '/data.mat'])
                local_set_status('Trajectory and Reference properties don''t match. Adjust first.');
                return;   
            else
                load([D.recon.recon_details.pname '/data.mat']);
                if size(data.wmap)~=D.recon.recon_details.recon_resolution
                    local_set_status('Trajectory and Reference properties don''t match. Adjust first.');
                    return;   
                end
            end
...
%%%
        ...
        if exist ([D.recon.recon_details.pname '/data.mat'],'file')
            d=load([D.recon.recon_details.pname '/data.mat']);
            if D.recon.recon_details.recon_resolution==size(d.data.wmap)
                D.reference = d.data;
            else
            ...
            data.cmaps=[];
            data.raw=[];
            save([D.recon.recon_details.pname '/data.mat'],'data');
            recon_details=D.recon.recon_details;
            save([D.recon.recon_details.pname '/recon_details.mat'],'recon_details');
        end
